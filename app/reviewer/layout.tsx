...
const NAV_GROUPS = [
  {
    ...
    items: [
      ...
      { href: '/reviewer/tinjauan-akhir', label: 'Tinjauan Akhir', icon: CheckSquare },
    ],
  },
  {
    label: 'Data',
...
function ReviewerBreadcrumb({ pathname }: { pathname: string }) {
  ...
  // [id] detail page: /reviewer/tiket/<id>[/tinjauan-akhir]
  ...
  if (isTicketDetail) {
    crumbs = [
      ...
      {
        href: isFinalReview ? '/reviewer/tinjauan-akhir' : '/reviewer/tinjauan-awal',
        label: isFinalReview ? 'Tinjauan Akhir' : 'Tinjauan Awal',
        isLast: false,
      },
      { href: pathname, label: isFinalReview ? 'Verifikasi Akhir' : 'Detail Tiket', isLast: true },
    ];
  } else {
      ...
      <DropdownMenuGroup>
        <DropdownMenuLabel className="flex items-center gap-4 px-2 py-2.5 font-normal">
          ...
          <div className="flex flex-1 flex-col items-start">
            <span className="text-base font-semibold">Ahmad Subagja</span>
            <span className="text-sm text-muted-foreground">a.subagja@brt.co.id</span>
          </div>
        </DropdownMenuLabel>
      </DropdownMenuGroup>
...
export default function ReviewerLayout({ children }: { children: React.ReactNode }) {
  ...
  const crumbsLastLabel = isTicketDetail
    ? isFinalReview ? 'Verifikasi Akhir' : 'Detail Tiket'
    : ROUTE_LABELS[pathname] ?? 'Dashboard';
  const openCount = MOCK_TICKETS.filter(t => t.status === 'OPEN' || t.status === 'REWORK_REQUIRED').length;
  const pendingCount = MOCK_TICKETS.filter(t => t.status === 'PENDING_REVIEW').length;
  const counts: Record<string, number> = {
    '/reviewer/tinjauan-awal': openCount,
    '/reviewer/tinjauan-akhir': pendingCount,
  };
                ...
                <SidebarGroupContent>
                  <SidebarMenu>
                    {group.items.map((item) => {
                      ...
                      const isActive =
                        item.href === '/reviewer' ? pathname === '/reviewer' : pathname.startsWith(item.href);
                      return (
          ...
          <header className="bg-card z-50 shrink-0 border-b">
            <div className="mx-auto flex max-w-360 items-center justify-between gap-6 px-4 py-2 sm:px-6">
              <div className="flex items-center gap-4">
                ...
                <ReviewerBreadcrumb pathname={pathname} />
