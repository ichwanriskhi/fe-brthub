      ...
      <DropdownMenuGroup>
        <DropdownMenuLabel className="flex items-center gap-4 px-2 py-2.5 font-normal">
          ...
          <div className="flex flex-1 flex-col items-start">
            <span className="text-base font-semibold">Tri Mentari</span>
            <span className="text-sm text-muted-foreground">t.mentari@brt.co.id</span>
          </div>
        </DropdownMenuLabel>
      </DropdownMenuGroup>
...
export default function UnitLayout({ children }: { children: React.ReactNode }) {
  ...
  // Tiket siap diberi handler: sudah ada prioritas & unit tujuan, belum ada handler
  const queueCount = MOCK_TICKETS.filter(
    (t) => t.priority !== null && t.priority !== undefined && !t.handlerName && t.status !== 'REJECTED' && t.status !== 'CLOSED'
  ).length;
  const counts: Record<string, number> = {
    '/unit/antrean': queueCount,
  };
                ...
                <SidebarGroupContent>
                  <SidebarMenu>
                    {group.items.map((item) => {
                      ...
                      const isActive =
                        item.href === '/unit' ? pathname === '/unit' : pathname.startsWith(item.href);
                      return (
          ...
          <header className="bg-card z-50 shrink-0 border-b">
            <div className="mx-auto flex max-w-360 items-center justify-between gap-6 px-4 py-2 sm:px-6">
              <div className="flex items-center gap-4">
                ...
                <UnitBreadcrumb pathname={pathname} />
