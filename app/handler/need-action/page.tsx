...
export default function HandlerNeedActionPage() {
  ...
  const openTickets = MOCK_TICKETS.filter((t) => {
    const isTargetStatus = t.status === 'OPEN';
  ...
  const totalPages = Math.max(1, Math.ceil(openTickets.length / TICKETS_PER_PAGE));
  const safePage = Math.min(currentPage, totalPages);
  const paginatedTickets = openTickets.slice((safePage - 1) * TICKETS_PER_PAGE, safePage * TICKETS_PER_PAGE);
      ...
      <Card className="gap-0 overflow-hidden p-0">
        ...
        {paginatedTickets.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-2 border-t py-12 text-center">
            ...
            <p className="text-sm font-medium">Tidak ada tiket yang perlu ditindaklanjuti</p>
            <p className="text-xs text-muted-foreground">Semua tiket telah ditangani.</p>
          </div>
        ) : (
