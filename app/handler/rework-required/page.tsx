...
export default function HandlerReworkRequiredPage() {
  ...
  const reworkTickets = MOCK_TICKETS.filter((t) => {
    const isTargetStatus = t.status === 'REWORK_REQUIRED';
  ...
  const totalPages = Math.max(1, Math.ceil(reworkTickets.length / TICKETS_PER_PAGE));
  const safePage = Math.min(currentPage, totalPages);
  const paginatedTickets = reworkTickets.slice((safePage - 1) * TICKETS_PER_PAGE, safePage * TICKETS_PER_PAGE);
      ...
      <Card className="gap-0 overflow-hidden p-0">
        ...
        {paginatedTickets.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-2 border-t py-12 text-center">
            ...
            <p className="text-sm font-medium">Tidak ada tiket perlu revisi</p>
            <p className="text-xs text-muted-foreground">Semua resolusi telah diperbaiki.</p>
          </div>
        ) : (
