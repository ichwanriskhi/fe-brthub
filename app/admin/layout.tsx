...
function AdminBreadcrumb({ pathname }: { pathname: string }) {
  ...
  if (isDetail && ticketSub) {
    crumbs = [
      ...
      { href: parentHref[ticketSub] ?? '/admin', label: parentLabel[ticketSub] ?? 'Tiket', isLast: false },
...
const ProfileDropdown = () => (
  <DropdownMenu>
    ...
    >
      ...
      <span className="absolute right-0 bottom-0 block size-2 rounded-full bg-primary ring-2 ring-card" />
    </DropdownMenuTrigger>
      ...
      <DropdownMenuGroup>
        <DropdownMenuLabel className="flex items-center gap-4 px-2 py-2.5 font-normal">
          ...
          <div className="flex flex-1 flex-col items-start">
            <span className="text-base font-semibold">Admin BRTHub</span>
            <span className="text-sm text-muted-foreground">admin@brt.co.id</span>
          </div>
        </DropdownMenuLabel>
      </DropdownMenuGroup>
      <DropdownMenuSeparator />
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const crumbsLastLabel = isDetail
    ? 'Detail Tiket'
    : ROUTE_LABELS[pathname] ?? 'Dashboard';

  // Badge count for "Perlu Tindakan" (pending reporter identification)
  const pendingCount = MOCK_PENDING_REPORTERS.filter((p) => p.status === 'PENDING').length;
                ...
                <SidebarGroupContent>
                  <SidebarMenu>
                    {group.items.map((item) => {
                      ...
                      const showBadge = item.href === '/admin/ticket/action' && pendingCount > 0;
                      return (
