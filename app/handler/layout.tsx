      ...
      <DropdownMenuGroup>
        <DropdownMenuLabel className="flex items-center gap-4 px-2 py-2.5 font-normal">
          ...
          <div className="flex flex-1 flex-col items-start">
            <span className="text-base font-semibold">Dimas P.</span>
            <span className="text-sm text-muted-foreground">d.pratama@brt.co.id</span>
          </div>
        </DropdownMenuLabel>
      </DropdownMenuGroup>
...
export default function HandlerLayout({ children }: { children: React.ReactNode }) {
  ...
  const segments = pathname.split('/').filter(Boolean);
  const isTicketDetail = segments[1] === 'ticket' && segments.length >= 3;
  const crumbsLastLabel = isTicketDetail ? 'Detail Tiket' : ROUTE_LABELS[pathname] ?? 'Dashboard';
  const needActionCount = MOCK_TICKETS.filter(t => t.status === 'OPEN').length;
  const waitingReviewCount = MOCK_TICKETS.filter(t => t.status === 'PENDING_REVIEW').length;
  const reworkCount = MOCK_TICKETS.filter(t => t.status === 'REWORK_REQUIRED').length;
  const counts: Record<string, number> = {
    '/handler/need-action': needActionCount,
    '/handler/waiting-for-review': waitingReviewCount,
    '/handler/rework-required': reworkCount,
  };
                ...
                <SidebarGroupContent>
                  <SidebarMenu>
                    {group.items.map((item) => {
                      ...
                      const isActive =
                        item.href === '/handler' ? pathname === '/handler' : pathname.startsWith(item.href);
                      return (
          ...
          <header className="bg-card z-50 shrink-0 border-b">
            <div className="mx-auto flex max-w-360 items-center justify-between gap-6 px-4 py-2 sm:px-6">
              <div className="flex items-center gap-4">
                ...
                <HandlerBreadcrumb pathname={pathname} />
